...
For the most extreme precipitation events (Pr $\geq$ 40 mm/day), there is a statistically significant increase along the northwest coast ($\geq$ 60$\%$), the Cascades ($\sim$ 50$\%$) and Northern Rockies ($\geq$ 60$\%$) by end-of-century. Significant increases are also apparent along the Klamath range in California of about 20-40$\%$ from \textsf{hist} to \textsf{end}. Changes in accumulated precipitation for these events are consistent with the change in their frequency (see Figure \ref{fig:difIndex2}).  {\color{red}With a projected increase of temperatures in this region of 4-5 K over the cool season, this increase is in excess of the 7\% per degree change that would be anticipated from the C-C relationship (Figure \ref{fig:discussIndex}a).  In this case, the probable cause of this excess is due to the intensification of the storm track along the coast discussed in section \ref{sec:ResultsMeanClimatology}. [Paul: Check]} 
...
\caption{Mean precipitation and other related indices from VR-CESM and reference datasets over 1980-2005. (Note: Grids with statistically significance difference between VR-CESM and references mean are marked with stippling.)}
\label{fig:histEval1}
\end{center}
\end{figure}
...
\caption{The mean precipitation (Pr), 2m average temperature (Tavg), and 2m relative humidity (RH) averaged over each time period. (Note: Grids with statistically significance difference for the RH and Pr are marked with stippling.)}
\label{fig:meanClm}
\end{center}
\end{figure}
