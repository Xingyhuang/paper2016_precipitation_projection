...
Before proceeding, we first investigate how well the model is able to represent the character of precipitation over the WUS.  Figures \ref{fig:histEval1} and Figure \ref{fig:histEval2} depict the spatial character of the indices defined in Table \ref{tab:table1}. Considering the uncertainty within the reference datasets, the mean of the references are used to get the difference from the model output. T-test is applied here with UW, CPC and NARR as the three statistical samples and the historical runs as two samples averaged over the whole period, determining at each spatial point whether VR-CESM is statistically equivalent to the references as stippled in \ref{fig:histEval1} and \ref{fig:histEval2}.
